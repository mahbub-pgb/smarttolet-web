...
export default function SignUp() {
  ...
  const handleVerify = async (e) => {
    ...
    try {
      ...
      if (res.profileComplete) router.push('/dashboard');
      else setStep(3);
    } catch (err) {
      setError(errMsg(err));
    } finally {
      ...
      <div className="card">
        ...
        {step === 1 && (
          <form onSubmit={handleRequest}>
            ...
            <input value={mobile} onChange={(e) => setMobile(e.target.value)} placeholder="01712345678" required />
        ...
        {step === 2 && (
          <form onSubmit={handleVerify}>
            ...
            <button type="button" className="btn btn-ghost block" onClick={() => setStep(1)} disabled={busy}>
              Change number
            </button>
          </form>
        )}
        ...
        {step === 3 && (
          <form onSubmit={handleProfile}>
            ...
            <input type="password" value={profile.password} onChange={(e) => setProfile({ ...profile, password: e.target.value })} required />
            <small className="muted">Min 8 chars, with uppercase, lowercase, and a number.</small>
            <button className="btn btn-primary block" disabled={busy}>
              {busy ? 'Saving…' : 'Finish'}
            </button>
          </form>
